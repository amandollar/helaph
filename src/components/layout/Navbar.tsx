    ...
    <>
      <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${isScrolled ? "py-3 bg-slate-800/90 backdrop-blur-md shadow-lg" : "py-6 bg-transparent"}`}>
        <div className="container mx-auto px-4 flex justify-center">
          <div className={`w-full md:w-[60%] lg:w-[40%] flex items-center justify-between text-white px-6 py-3 rounded-full shadow-lg transition-all duration-300 ${isScrolled ? "bg-slate-800/80 backdrop-blur-sm border border-slate-600/50" : "bg-slate-900/60 backdrop-blur-sm border border-slate-700/30"}`}>
            {/* Logo */}
        ...
        {/* Mobile Menu */}
        <div className={`md:hidden fixed top-0 left-0 w-full h-screen bg-slate-900/95 backdrop-blur-lg flex flex-col items-center justify-center transition-all duration-300 ease-in-out ${isMobileMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
          <button
          ...
          <ul className="flex flex-col items-center gap-12 text-4xl">
            ...
            <li className={`group transform transition-all duration-700 ease-out hover:scale-105 ${isMobileMenuOpen ? 'animate-fade-in-up' : 'opacity-0 translate-y-4'}`} style={{ animationDelay: '0.4s' }}>
              ...
              <li className={`group transform transition-all duration-700 ease-out hover:scale-105 ${isMobileMenuOpen ? 'animate-fade-in-up' : 'opacity-0 translate-y-4'}`} style={{ animationDelay: '0.6s' }}>
                    <Link href="#contact" className="font-indie font-normal text-slate-300 hover:text-yellow-400 transition-all duration-700 hover:tracking-widest relative block tracking-wider" onClick={() => setIsMobileMenuOpen(false)}>
                        Contact
                        <span className="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-0 h-1 bg-gradient-to-r from-yellow-400 to-yellow-500 group-hover:w-full transition-all duration-900 ease-out"></span>
                    </Link>
              </li>
